...
namespace AutoFake.UnitTests
{
    public class FakeProcessorTests
    {
		...
		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_Mock_PreparedForInjecting(IMemberVisitorFactory factory)
		{
			Expression<Action<TestClass>> expr = t => t.SimpleMethod();
			var mock = new Mock<IMock>();

			_fakeProcessor.ProcessMethod(new[] { mock.Object }, new InvocationExpression(factory, expr));

			mock.Verify(m => m.BeforeInjection(It.Is<MethodDefinition>(met => met.Name == nameof(TestClass.SimpleMethod))));
		}

		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_IsInstalledInstruction_Injected(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_IsAsyncMethod_RecursivelyCallsWithBody(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_MembersFromTheSameModule_Success(IMemberVisitorFactory factory)
		{
			Expression<Action<TestClass>> expr = t => t.GetTestClass(Arg.IsAny<TestClass>(), Arg.IsAny<int>());

			_fakeProcessor.ProcessMethod(new[] { Mock.Of<IMock>() }, new InvocationExpression(factory, expr));
		...
		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_CtorFromTheSameModule_Success(IMemberVisitorFactory factory)
		{
		 ...
		 FakeProcessor generator)
		{
			...
			Assert.Throws<InvalidOperationException>(() =>
			 generator.ProcessMethod(new[] { Mock.Of<IMock>() }, new InvocationExpression(null, expr)));
		}
		...
		public void ProcessSourceMethod_NullMethod_Throws()
		{
			...
			Assert.Throws<InvalidOperationException>(() => _fakeProcessor.ProcessMethod(new[] { Mock.Of<IMock>() },
			 new InvocationExpression(null, expr)));
		}

		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_MethodWhichCallsMethodWithoutBody_DoesNotThrow(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessSourceMethod_RecursiveMethod_Success(IMemberVisitorFactory factory)
		{
			var proc = CreateProcessor<object>(new FakeOptions { DisableVirtualMembers = true });

			Expression<Action<TestClass>> expr = t => t.ToString();

			proc.Proc.ProcessMethod(new[] { Mock.Of<IMock>() }, new InvocationExpression(factory, expr));
		}

		[AutoMoqData, Theory]
		internal void ProcessSourceMethod_VirtualMethodWithSpecification_Success(
			IMemberVisitorFactory factory,
			[InjectModule] Mock<ITypeInfo> typeInfo)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessMethod_GenericArgsOnBothLevels_Success(IMemberVisitorFactory factory)
		{
			...
			var mock = new FakeMock(new ReplaceMock(new ProcessorFactory(proc.TypeInfo, proc.Writer), new InvocationExpression(factory, expr2)));

			proc.Proc.ProcessMethod(new[] { mock }, new InvocationExpression(factory, expr1));

			mock.AtLeastOneSuccess.Should().BeTrue();
		}

		[Theory, AutoMoqData]
		internal void ProcessMethod_GenericArgsOnSecondLevel_Success(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessMethod_FieldGenericArgs_Success(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessMethod_NestedMethodGenericArg_Success(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessMethod_NestedFieldGenericArg_Success(IMemberVisitorFactory factory)
		{
		...
		[Theory, AutoMoqData]
		internal void ProcessMethod_FieldGenericArgsOnBothLevels_Success(IMemberVisitorFactory factory)
		{
		...
		private static (ITypeInfo TypeInfo, FakeProcessor Proc, AssemblyWriter Writer) CreateProcessor<T>(FakeOptions options)
		{
			...
			var factory = new Mock<IMemberVisitorFactory>();
			factory.Setup(f => f.GetMemberVisitor<GetTestMethodVisitor>()).Returns(new GetTestMethodVisitor());
			var fakeProcessor = new FakeProcessor(typeInfo, assemblyWriter, options, factory.Object);
